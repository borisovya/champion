...
<template>
  <div class=" flex flex-column">
    ...
    <div v-if="!isLoading" class="flex flex-column justify-content-between">
        ...
        <Column field="newsImg" header="Изображение" style="width: 12rem">
          <template #body="slotProps">
            <div class="flex align-content-center">
              <img :src="slotProps.data.newsImg" :alt="slotProps.data.newsImg" class=" h-4rem border-round overflow-hidden"/>
            </div>
          </template>
        </Column>
        ...
        <Column header="" style="width: 8rem">
          ...
          <template v-slot:body="{data}">
            ...
            <Button text rounded size="large" icon="pi pi-eye" severity="secondary"
                    @click="route.push(`/admin/news/show/${data.id}`)"></Button>
          </template>
        </Column>
      ...
      <Paginator :rows="filters.rows" :totalRecords="news.length" :rowsPerPageOptions="[5, 10, 15]"/>
    </div>
  </div>
</template>
