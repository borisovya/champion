  ...
  try {
    ...
    if (!res || !categoryList) {
      toast.add({
        severity: 'error',
        summary: 'Ошибка',
        detail: 'Загрузить данные не удалось.',
        life: 3000
      })
      navigate.back()
      return
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (!isValid) {
      ...
      if (updateProductRes) {
        const pickUpload = await productBindImage(
          (updateProductRes as Product).id,
          productFieldsData.image
        )
        toast.add({
          severity: 'success',
          summary: 'Готово',
    ...
    <form v-else class="p-grid p-fluid p-justify-center" @submit.prevent.stop="onSubmit">
      ...
      <div class="lg:flex border-round inputBlocksPaddingTop">
        <div class="p-2 flex flex-column align-items-start justify-content-center lg:w-12">
          ...
          <Image
            :src="
              productFieldsData.imgUrl ? productFieldsData.imgUrl : asset(productFieldsData.image)
            "
            alt="Image"
            width="200"
